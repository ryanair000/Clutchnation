...
import type { NormalizedPlatformProfile, PlatformType } from '@/types';
import Image from 'next/image';
import { PLATFORM_LABELS, PLATFORM_ICONS } from '@/lib/constants';

interface PlatformPreviewCardProps {
  platform: PlatformType;
  profile: NormalizedPlatformProfile | null;
  loading: boolean;
  error: string | null;
  notFound: boolean;
  onConfirm: () => void;
...
export function PlatformPreviewCard({
    ...
    return (
      <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
        <p className="text-sm text-amber-800">
          We couldn&apos;t verify this {label} account right now. You can still
    ...
    return (
      <div className="mt-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3">
        <p className="text-sm text-red-700">
          No {label} account found with this username.
        </p>
        <button
          type="button"
          onClick={onCancel}
  ...
  // PSN-specific: trophy counts
  const trophyCounts = profile.profileData?.trophyCounts as
    | Record<string, number>
    | undefined;
  const isPlus = profile.profileData?.isPlus as boolean | undefined;
  const trophyLevel = profile.profileData?.trophyLevel as number | undefined;

  // Xbox-specific: gamerscore
  const gamerscore = profile.profileData?.gamerscore as number | undefined;

  // Steam-specific
  const gameCount = profile.profileData?.gameCount as number | undefined;

  const totalTrophies = trophyCounts
          ...
          <div className="mt-0.5 flex items-center gap-2 text-xs text-ink-light">
            ...
            {gamerscore != null && (
              <span className="flex items-center gap-1 font-medium">
                ...
                {gamerscore.toLocaleString()} Gamerscore
              </span>
            )}
            {gameCount != null && (
      ...
      {trophyCounts && (
        <div className="flex items-center gap-3 border-t border-surface-200/60 bg-surface-50/50 px-4 py-2">
          {(['platinum', 'gold', 'silver', 'bronze'] as const).map((tier) => {
            ...
            const count = trophyCounts[tier] ?? 0;
            return (
              <span
                key={tier}
